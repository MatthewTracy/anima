...
|---|---|
| v1.1.75 | Three more agent-IQ fixes, all surfaced by the v1.1.74 Forum game: (1) talking-nudge — when ≥3 of an agent's last 5 commands were `!factionChat`, the next prompt gets a system note pushing them toward a world-affecting command (v1.1.74 still had ~30% factionChat share). (2) credit kills from chat — mineflayer's `entityDead` correlation doesn't reliably fire for PvP kills (the v1.1.74 game logged 86 damage / 3 combat_death / **kills: 0**); a new `extractKillerFromDeathMessage` parser catches "X was slain by <me>" from chat. (3) cosmetic — move the SOFT_FAIL retry check above the "Generated response" log line so the run log no longer prints the sentinel verbatim on retries. +10 tests |
| v1.1.74 | Agent-IQ pack — three behavioral fixes from the v1170 log audit. (A) Falsy `executeCommand` results now leave a "Command X returned no result, try a different approach" system note in history — pre-fix an agent retried the same dead `!goToCoordinates` 16+ times because failure was silent. (B) `election_called` / `nomination` broadcasts via the witness pipeline with a distance-bypassing announcement path — pre-fix a presidential election ran 78s and timed out at "no votes cast" because nobody outside the caller's faction-chat knew. (C) `promptConvo` retries on the `SOFT_FAIL_RESPONSE` sentinel — pre-fix a single null-content reply from OpenRouter became the agent's whole response and the agent went idle for the rest of the game. +8 tests |
| v1.1.73 | Seed 12345 → 0: with the seed mechanism actually working (v1.1.72), 12345 turned out to put world spawn in ocean biome — a live run had Madison/Hamilton spawn at `biome: ocean`, drown, and never recover. Seed 0 puts origin in forest with the full Constitutional↔Anarchy corridor (-120…+120 at z=0) and the ±40 perpendicular strips at each meeting ground all solid land. Probed via rcon |
| v1.1.72 | Rename `LEVEL_SEED` → `SEED` in docker-compose: itzg/minecraft-server reads `SEED`. Pre-fix the env var was silently ignored, `server.properties` had `level-seed=` empty, and every regenerated world was random — the "fixed seed = same map every game" claim was a lie |
